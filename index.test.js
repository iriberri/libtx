...
const {create, toBuffer, fromBuffer} = require('./src')
...
function assertCompile (operations, config = {defaults: true, debug: false}) {
  const {errors, structs} = create(operations, Types(config))
...
function assertSerializer (type, value) {
  ...
  deepEqual(type.toObject(obj), type.toObject(obj2), 'serialize buffer')
}
