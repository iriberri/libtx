...
*/
module.exports = (config = {defaults: false, debug: false, customTypes: {}}) => {
...
const vector = validation => {
  ...
  return {
    fromByteBuffer (b) {
      ...
      return sortOperation(result, validation.type)
    },
    appendByteBuffer (b, value) {
      validate(value, validation)
      value = sortOperation(value, validation.type)
...
const time = (validation) => {
  const _time = {
    ...
    fromObject (value) {
      ...
      // Chrime assumes Zulu when missing, Firefox does not
...
const sortOperation = (array, stOperation) => {
  ...
  // console.log('operation.nosort', stOperation.nosort)
  return stOperation.nosort ? array
    : stOperation.compare
    ? array.sort((a, b) => stOperation.compare(firstEl(a), firstEl(b))) // custom compare operation
    : array.sort((a, b) =>
