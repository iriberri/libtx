...
/**
  @summary Create a serializer for each operation.
  @return {CreateStruct}
*/
function create (operations, types, config = types.config) {
  const errors = []
...
function toBuffer (type, value) {
  return Buffer.from(toByteBuffer(type, value).toBinary(), 'binary')
}
